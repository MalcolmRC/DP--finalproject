...
# Sentiment Analysis with negation using sentimentr (https://www.r-bloggers.com/2019/07/how-to-do-negation-proof-sentiment-analysis-in-r/) ----
sentiment_sentence <- vector("list", length(brookings_list))
negation_plot <- vector("list", length(brookings_list))
avg_sentiment <- vector("list", length(brookings_list))

for (i in seq_along(brookings_list)) {
  
  sentiment_sentence[[i]] <- sentiment(brookings_list[[i]])
  avg_sentiment[[i]] <- sentiment_sentence[[i]]$sentiment %>% mean() 
  names(avg_sentiment) <- articles
  
  negation_plot[[i]] <- ggplot(data = sentiment_sentence[[i]]) +
      geom_col(aes(x = sentence_id, y = sentiment)) +
      ggtitle(glue("Brookings {articles[[i]]} Article Overall Negation Sentiment by Sentence"))

}

plot_grid(plotlist = negation_plot, nrow = 2, ncol = 1)
(avg_sentiment_df <- as.data.frame(avg_sentiment))
